            ...
            for (j = 0; j < i["feed"]["entry"]["length"]; j = j + 1) {
                ...
                if (j === 0 && !$("body")["hasClass"]("error_page")) {
                    ...
                    for (r = 0; r < n["length"]; r = r + 1) {
                        if (p["indexOf"](n[r]) == -1) {
                            if (r == h && q["indexOf"]("post-preview") == -1 && q["indexOf"]("www.blogger") == -1 && q["indexOf"]("b/layout-preview") == -1 && q["indexOf"]("b/preview") == -1 && q["indexOf"]("translate.google") == -1 && q["indexOf"]("webcache.googleusercontent") == -1 && q["indexOf"]("template-editor") == -1) {
...
$(".post-feed-7 .HTML").each(function() {
    ...
    $.ajax({
        ...
        success: function(json) {
            ...
            htmlcode += '</ul>';
            document.getElementById('post-feed7').innerHTML = htmlcode;
            $('.post-feed-thumbnail-1 a').attr('style', function(i, src) {
                return src.replace('/s72-c', '/s1600')
            });
            $('.post-feed-thumbnail-1 a').attr('style', function(i, src) {
...
$(".post-feed-8 .HTML").each(function() {
    ...
    $.ajax({
        ...
        success: function(json) {
            ...
            htmlcode += '</div></div>';
            document.getElementById('post-feed8').innerHTML = htmlcode;
            $('.post-feed-thumbnail-5 a').attr('style', function(i, src) {
                return src.replace('/s72-c', '/s1600')
            });
            $('.post-feed-thumbnail-5 a').attr('style', function(i, src) {
...
$(".post-feed-9 .HTML").each(function() {
    ...
    $.ajax({
        ...
        success: function(json) {
            ...
            htmlcode += '</ul>';
            document.getElementById('post-feed9').innerHTML = htmlcode;
            $('.post-feed-thumbnail-6 a').attr('style', function(i, src) {
                return src.replace('/s72-c', '/s1600')
            });
            $('.post-feed-thumbnail-6 a').attr('style', function(i, src) {
...
$(document).ready(function() {
    (function(e) {
        ...
        e.get(t.attr("href"), function(n) {
            t.html(' <strong><i class="fa fa-angle-right"></i>التالى</strong><br/><div class="post-pager-title"><span>' + e(n).find(".post .post-title").text() + '</span></div>')
        }, "html");
...
$('.ContactForm').appendTo('.blogger-items-contact') $('.popular-posts li').each(function() {
...
$(".hide-comments").click(function() {
    $(".comments").slideToggle(), $(".zer1").toggle(), $(".zer2").toggle()
});
...
$(".avatar-image-container img").attr('src', function(i, src) {
    return src.replace('//img1.blogblog.com/img/blank.gif', '//3.bp.blogspot.com/-fgwrcZWeRrU/V26tvNcGtsI/AAAAAAAAAG4/lGwGnQDZsNY7bAPr8hVorZruD-jHHxxOgCLcB/s50/anonyme.png')
});
...
function() {
    for (var a = /(\[img\])?((http:|https:)?\/\/\S*?\.(jpg|gif|png|bmp|jpeg]))(\[\/img\])?/gi, b = /(\[vid\])?http(?:s?):\/\/(?:www\.)?youtu(?:be\.com\/watch\?v=|\.be\/)([\w\-\_]*)(&(amp;)?‌​[\w\?‌​=]*)?(\[\/vid\])?/gi, c = /(\[vid\])?(http:|https:)?\/\/(vimeo.com)\/([0-9]*)(\[\/vid\])?/gi, d = document.querySelectorAll(".comment-content"), e = 0; e < d.length; e++) {
        for (var f = d[e].getElementsByTagName("a"), g = 0; g < f.length; g++) if (f[g].href.match(a) || f[g].href.match(b) || f[g].href.match(c)) {
        ...
        var j = d[e].innerHTML;
        j = j.replace(a, '<img style="max-width: 100%; height: auto;display: blocK;margin: 10px auto;border: 5px solid transparent; outline: 1px solid #eeeeee4d;" src="$2"" alt=""/>'), j = j.replace(b, '<div style="position:relative;width:100%;height:0;padding-bottom:56.25%;overflow:hidden;margin:20px auto;border: 5px solid transparent; outline: 1px solid #eeeeee4d;"><iframe style="position: absolute;width: 100%;height: 100%;top: 0;left: 0;" src="https://www.youtube.com/embed/$2" frameborder="0" allowfullscreen></iframe></div>'), j = j.replace(c, '<div style="position:relative;width:100%;height:0;padding-bottom:56.25%;overflow:hidden;margin:20px auto;border: 5px solid transparent; outline: 1px solid #eeeeee4d;"><iframe style="position: absolute;width: 100%;height: 100%;top: 0;left: 0;" src="https://player.vimeo.com/video/$4" frameborder="0" allowfullscreen></iframe></div>'), d[e].innerHTML = j
    }
}();
$(document).ready(function() {
    ...
    function n() {
        ...
        function() {
            ...
            n(l), s = i.length, setTimeout(function() {
                for (t = 0; s > t; t++) if (o = r(i[t]), null == o.offsetParent || "none" == (e.getComputedStyle ? a.defaultView.getComputedStyle(o, null).getPropertyValue("display") : o.currentStyle.display)) return d("#" + i[t])
            }, 250)
        }(), function() {
            var e, t = r(0, "img"),
            ...
            var n, i, s, l, c, f = {
                ...
                u = r(0, "script"),
                h = u.length - 1;
            for (a.write = null, s = h; s >= 0; --s) if (c = u[s], typeof f[c.src] != o) {
...
$(document).ready(function() {
    var t = $(".topic-nav .next"),
        a = $(".topic-nav .prev") $.get(t.attr("href"), function(a) {
            var r = $(a).find("h1.entry-title").text(),
                i = $(a).find(".post .separator img") try {
                    var n = '<img src="' + i[0].src + '"alt="' + r + '"/>'
                } catch (c) {
                    var n = '<img src="' + noimg + '"alt="' + r + '"/>'
                }
            t.html('<span class="next-txt">' + next_text + "</span><h4>" + r + '</h4><div class="topic-img">' + n + "</div>")
        }, "html"),
        $.get(a.attr("href"), function(t) {
            var r = $(t).find("h1.entry-title").text(),
                i = $(t).find(".post .separator img") try {
                    var n = '<img src="' + i[0].src + '"alt="' + r + '"/>'
                } catch (c) {
                    var n = '<img src="' + noimg + '"alt="' + r + '"/>'
                }
...
function() {
    var e = document.createElement("script") e.type = "text/javascript",
        e.async = !0,
        e.src = "//cdn.jsdelivr.net/gh/abdelalilebbihi/abdou_tech/gadget/download-severs.js"
